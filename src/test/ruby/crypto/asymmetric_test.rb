...
module Crypto
  class AsymmetricTest < Test::Unit::TestCase
    ...
    def test_initialize
      pubkey = File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/public.der' )
      privkey = File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/private.pem' )
      privpass = 'iloveyou'
    ...
    def test_set_public_key
      pubkey = OpenSSL::PKey::RSA.new( File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/public.der' ) )
      @asim.public_key = pubkey
      assert_not_nil( @asim.public_key )
      assert( @asim.public_key.is_a?( OpenSSL::PKey::RSA ) )
      assert( @asim.public_key.public? )

      privkey = OpenSSL::PKey::RSA.new( File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/private.pem' ), 
                                        'iloveyou' )
      ...
      @asim.public_key = File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/public.der' )
      assert_not_nil( @asim.public_key )
      assert( @asim.public_key.is_a?( OpenSSL::PKey::RSA ) )
      assert( @asim.public_key.public? )

      @asim.public_key = File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/private.pem' ), 'iloveyou'
      assert_not_nil( @asim.public_key )
      assert( @asim.public_key.is_a?( OpenSSL::PKey::RSA ) )
      assert( @asim.public_key.public? )
    end
    ...
    def test_set_private_key
      ...
      @asim.private_key = File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/private.pem' ), 'iloveyou'
      assert_not_nil( @asim.private_key )
      assert( @asim.private_key.is_a?( OpenSSL::PKey::RSA ) )
      assert( @asim.private_key.private? )

      @asim.private_key = File.read( ENV['CUJAU_CRYPTO_HOME'] + '/src/test/resources/public.der' )
      assert_nil( @asim.private_key )
    end
